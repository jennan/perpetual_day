...
from src.models import UNet2DModel
...
if __name__ == "__main__":
    ...
    resultsdir = Path("results/unet2_diff_004")
    modelpath = resultsdir / "lightning_logs/version_0/checkpoints/last.ckpt"

    model = UNet2DModel.load_from_checkpoint(modelpath)
    ...
    featurepipe_nocache["20200301T0000"]
    ...
    targetpipe_nocache["20200301T0000"]
    ...
    for date in train_dates[:5]:
        ...
        figname = resultsdir / f"train_{date.datetime:%Y%m%dT%H%M}.png"
    ...
    for date in valid_dates[:5]:
        ...
        fig = plot_results(features, targets, preds)
        figname = resultsdir / f"val_{date.datetime:%Y%m%dT%H%M}.png"
        fig.savefig(figname, bbox_inches="tight")
